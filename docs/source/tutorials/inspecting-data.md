...
Since 0.8.0 each run has a directory of its own, named for the participant,
the session and the moment the run started, and every file inside repeats all
three. A repeated session can no longer overwrite an earlier one.

For a participant named `0341` in session `001`, a completed run looks like
this:

```text
data/sub-0341/ses-001/run-20260828-142530/
├── manifest.json
├── session.log
├── sub-0341_ses-001_run-20260828-142530_behaviour.txt
├── sub-0341_ses-001_run-20260828-142530_final.txt
├── sub-0341_ses-001_run-20260828-142530_signal.txt
├── sub-0341_ses-001_run-20260828-142530_posterior-Intero.npy
├── sub-0341_ses-001_run-20260828-142530_posterior-Extero.npy
├── sub-0341_ses-001_run-20260828-142530_ppg-20.txt
└── sub-0341_ses-001_run-20260828-142530_parameters.pickle
```

The rolling trial log is the `_behaviour.txt` file. Cardioception rewrites it
after every trial, which makes it the recovery copy if a session is
interrupted. On normal completion the same table is written as `_final.txt`.
Prefer `_final.txt` when it exists.

`manifest.json` is written at the *start* of the session and records the
configuration the run was launched with, so an aborted run still says what it
was going to be.

| File | What it contains | Use during collection |
|---|---|---|
| `manifest.json` | The configuration, written before the first trial | Confirm what an aborted run was set up to do |
| `*_behaviour.txt` | Rolling trial-level log | Recover an interrupted session |
| `*_final.txt` | Completed trial-level log | Main file for the checks on this page |
| `*_posterior-Intero.npy`, `*_posterior-Extero.npy` | Joint Psi posterior after each adaptive trial | Check uncertainty and parameter-grid boundaries |
| `*_signal.txt` | PPG samples collected during cardiac listening intervals, with trial numbers and sample times | Investigate suspicious trial-level heart rates |
| `*_ppg-*.txt` | Pulse-oximeter segments saved at breaks and at the end | Inspect the physiological recording in detail |
| `*_parameters.pickle` | Parameters used for that session | Confirm the intended task settings |
| `session.log` | Everything the run logged, including rejected heart rates | Work out why a trial behaved oddly |

Match files by suffix rather than assuming a fixed prefix: the prefix carries
the participant, session and run identifiers passed to `getParameters()`.
...
session_file = Path("data/sub-0341/ses-001/run-20260828-142530")
# Match by suffix: the run identifier is a timestamp.
session_file = next(session_file.glob("*_final.txt"))
...
run_dir <- "data/sub-0341/ses-001/run-20260828-142530"
session_file <- list.files(run_dir, pattern = "_final\.txt$", full.names = TRUE)[1]
