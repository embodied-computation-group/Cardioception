...
single_sub_analysis <- function(df, interoPost = NA, exteroPost = NA, bayesian = FALSE, model = NA, out) {
  ...
  dir.create(paste0(out,"/results_sub"))
  ...
  # save the plot:
  ggsave(paste0(out,"/results_sub/resultplot_basic.png"), plot, width = 4000, height = 2200, units = "px")
  ...
  if (bayesian == TRUE) {
    if (n_mod == 2) {
      ...
      # save the figures
      ggsave(paste0(out,"/results_sub/resultplot_bayse_intero.png"), baysplot_in, width = 4000, height = 2200, units = "px")
      ggsave(paste0(out,"/results_sub/resultplot_bayse_extero.png"), baysplot_ex, width = 4000, height = 2200, units = "px")
      ggsave(paste0(out,"/results_sub/resultplot_bayse.png"), baysplot, width = 4000, height = 2200, units = "px")
    ...
    # save it
    write.csv(resultsdata, paste0(out,"/results_sub/data.csv"))
...
study_analysis <- function(path, bayesian, model, folder = T, out) {
  if (folder) {
    ...
    sub_files <- list.files(path = path, full.names = TRUE, recursive = FALSE, pattern = "*.txt")
