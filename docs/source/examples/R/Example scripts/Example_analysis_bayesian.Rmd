...
```{r message=FALSE}
pacman::p_load(tidyverse,ggdist,psycho,caret,patchwork, gt, cowplot, grid,reticulate,cmdstanr,posterior,rstan,bayesplot,here)
np <- import("numpy")
```
...
psychophysics_df = read_csv('https://github.com/LegrandNico/CardioceptionPaper/raw/main/data/Del2_merged.txt')
...
## The out flag will determine where the results of the analysis will be put

```{r message=FALSE, results='hide',warning=FALSE}
model = cmdstan_model(here("docs","source","examples","R","src","first_model.stan"))

results = single_sub_analysis(df, interoPost = NA, exteroPost = NA, bayesian = T, model = model, out = here::here("docs","source","examples","R"))
```
...
data = study_analysis(path = path_to_data, bayesian = T, model = model, folder = T, out = out)
```
